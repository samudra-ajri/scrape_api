...
class DigilearnPodcastSpider(scrapy.Spider):
    global auth, CLEANR
    settings = get_project_settings()

    name = 'digilearn_podcast'
    ...
    def start_requests(self):
        yield scrapy.Request(
            url='https://service.mydigilearn.id/v2/podcasts/list?perpage=50&page=1&order_by=latest',
            callback=self.parse,
            headers={
                'Authorization': auth
            }
    ...
    def parse(self, response):
        ...
        sites = []
        for item in items:
            url = f'https://service.mydigilearn.id/v2/podcasts/{item.get("id")}'
            sites.append(url)
        ...
        if next_page <= total_pages:
            yield scrapy.Request(
                url=f'https://service.mydigilearn.id/v2/podcasts/list?perpage=50&page={next_page}&order_by=latest',
                callback=self.parse,
                headers={
                    'Authorization': auth
                }
    ...
    def parse_2(self, response):
        ...
        yield {
            ...
            'url': f'https://www.mydigilearn.id/podcast-detail/{item.get("id")}',
