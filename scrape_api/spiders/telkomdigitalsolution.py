...
class TelkomdigitalsolutionSpider(scrapy.Spider):
    global key, channel_id
    settings = get_project_settings()
    
    name = 'telkomdigitalsolution'
    allowed_domains = ['youtube.googleapis.com']

    key = settings.get('YOUTUBE_API_KEY')
    channel_id = settings.get('YOUTUBE_CHANNEL_ID_TELKOMDIGITAL')
    ...
    def parse(self, response):
        ...
        for item in items:
            yield {
                ...
                'content_type': 'telkomdigitalsolution',
