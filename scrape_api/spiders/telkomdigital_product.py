...
class telkomdigitalProductSpider(scrapy.Spider):
    global token, uid, client_id, CLEANR
    settings = get_project_settings()

    name = 'telkomdigital_product'
    ...
    def start_requests(self):
        yield scrapy.Request(
            url='https://admin.telkomdigital.id/api/v2/products?limit=50&page=1',
            callback=self.parse,
            headers={
                'access-token': token,
                'client': client_id,
    ...
    def parse(self, response):
        ...
        sites = []
        for item in items:
            url = f'https://admin.telkomdigital.id/api/v1/products/{item.get("id")}'
            sites.append(url)
        ...
        if next_page:
            yield scrapy.Request(
                url=f'https://admin.telkomdigital.id/api/v2/products?limit=50&page={next_page}',
                callback=self.parse,
                headers={
                    'access-token': token,
                    'client': client_id,
    ...
    def parse_2(self, response):
        resp = json.loads(response.body)
        item = resp.get('product')
        yield {
            'title': item.get('name'),
            'description': re.sub(CLEANR, '', item.get('description')) if item.get('description') else item.get('title'),
            'url': f'https://telkomdigital.id/catalog/{item.get("name").lower().replace(" ", "-")}',
            'img_url': f'https://admin.telkomdigital.id{item.get("logo")}',
